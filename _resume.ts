...
import type lumeCMS from "lume/cms/mod.ts";

type CMS = ReturnType<typeof lumeCMS>;
...
/**
 * Register all resume-related CMS documents and collections.
 */
export function registerResumeCMS(cms: CMS): void {
  // --- Resume Data Documents ---
  cms.document({
    name: "general-resume-data",
    description: "Information that is the same across resumes",
    store: "src:resumes/_data/general.yml",
    fields: generalResumeDataFields,
  });
  ...
  cms.document({
    ...
    "fields": summaryResumeDataFields,
  });
      ...
      "description: text!",
      {
        ...
        init: (field, content) => {
          const possible_layouts = (Array.from(
            ...
            .filter((c) => c.type === "file" && !c.name.startsWith("resume"))
            .map(
              (c) => {
                return {
                  value: c.path,
        ...
        fields: [
          {
            ...
            init(field, { data }) {
              ...
              ).flatMap((e) => (e as { roles: { key: string } }).roles).map(
                (r) => r.key,
              );
            },
          },
